...
public class BlockTileRenderer : MonoBehaviour
{
	...
	protected class RenderInfo
	{
		...
		{
			new RenderInfo(renderer, queryLayer, renderLayer, def, element, isBlueprint: false);
		}
		...
		public RenderInfo(BlockTileRenderer renderer, int queryLayer, int renderLayer, BuildingDef def, SimHashes element, bool isBlueprint)
		{
			...
			material.SetFloat("_packedParameters", num);
			int num2 = Grid.WidthInCells / 16 + 1;
			int num3 = Grid.HeightInCells / 16 + 1;
			meshChunks = new Mesh[num2, num3];
			dirtyChunks = new bool[num2, num3];
			...
			{
				decorRenderInfo = new DecorRenderInfo(num2, num3, queryLayer, def, blockTileDecorInfo);
			}
			int num4 = def.BlockTileAtlas.items[0].name.Length - 4 - 8;
			int startIndex = num4 - 1 - 8;
	...
	{
		AddBlock(renderLayer, def, isReplacement, element, cell, isBlueprint: false);
	}
	...
	public void AddBlock(int renderLayer, BuildingDef def, bool isReplacement, SimHashes element, int cell, bool isBlueprint)
	{
		...
		if (!renderInfo.TryGetValue(key, out var value))
		{
			...
			value = new RenderInfo(this, queryLayer, renderLayer, def, element, isBlueprint);
