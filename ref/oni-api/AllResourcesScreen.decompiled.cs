...
public class AllResourcesScreen : ShowOptimizedKScreen, ISim4000ms, ISim1000ms
{
	...
	public void RefreshRows()
	{
		...
		foreach (KeyValuePair<Tag, CategoryRow> categoryRow in categoryRows)
		{
			...
			{
				float calories = WorldResourceAmountTracker<RationTracker>.Get().CountAmount(null, ClusterManager.Instance.activeWorld.worldInventory);
				if (categoryRow.Value.CheckAvailableAmountChanged(amount, updateIfTrue: true))
				{
	...
	private void RefreshCharts()
	{
		...
		foreach (KeyValuePair<Tag, CategoryRow> categoryRow in categoryRows)
		{
			...
			if (resourceStatistic != null)
			{
				...
				if (array.Length != 0)
				{
					sparkLayer.graph.axis_x.max_value = array[^1].first;
				}
				else
				{
		...
		foreach (KeyValuePair<Tag, ResourceRow> resourceRow in resourceRows)
		{
			...
			if (resourceStatistic2 != null)
			{
				...
				if (array2.Length != 0)
				{
					sparkLayer2.graph.axis_x.max_value = array2[^1].first;
				}
				else
				{
