...
public class KAnimBatchManager
{
	...
	public static int[] AtlasNames = new int[16]
	{
	...
	public static int[] MaxAtlasesByMaterialType = new int[8] { 12, 12, 12, 16, 12, 13, 12, 12 };
	...
	public void Register(KAnimConverter.IAnimConverter controller)
	{
		...
		if (!batchSets.TryGetValue(batchKey, out var value))
		{
			...
			if (value.key.materialType == KAnimBatchGroup.MaterialType.UI || controller.IsAlwaysVisible())
			{
	...
	public void RenderKAnimPostProcessingEffects(KAnimConverter.PostProcessingEffects effectsToRender)
	{
		...
		foreach (KAnimConverter.PostProcessingEffects key in KAnimConverter.ShaderNameForPostProcessingEffect.Keys)
		{
			if ((effectsToRender & key) == 0)
			{
				continue;
			}
			...
			foreach (BatchSet activeBatchSet in activeBatchSets)
			{
				...
				for (int i = 0; i < activeBatchSet.batchCount; i++)
				{
					...
					bool flag = false;
					foreach (KAnimConverter.IAnimConverter controller in batch.Controllers)
					{
						if ((controller.GetPostProcessingEffectsCompatibility() & key) != 0)
						{
							flag = true;
							break;
						}
					...
					if (flag)
					{
						Vector3 zero = Vector3.zero;
						zero.z = batch.position.z + num;
						Graphics.DrawMesh(mesh, zero, Quaternion.identity, activeBatchSet.group.GetPostProcessingMaterial(key), layer, null, 0, batch.matProperties);
					}
				}
			}
		}
