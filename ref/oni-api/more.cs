...
public class BuildingDef : Def, IHasDlcRestrictions
{
	...
	public IReadOnlyList<string> SearchTerms => searchTerms;
	...
	{
		return IsAreaClear(source_go, cell, orientation, layer, tile_layer, replace_tile, restrictToActiveWorld: true, out fail_reason, !PreventBuildOverPlants);
	}
	...
	private bool IsAreaClear(GameObject source_go, int cell, Orientation orientation, ObjectLayer layer, ObjectLayer tile_layer, bool replace_tile, bool restrictToActiveWorld, out string fail_reason, bool permitUproots = true)
	{
		...
		for (int i = 0; i < PlacementOffsets.Length; i++)
		{
			...
			if (!num2)
			{
				...
				if (gameObject2 != null)
				{
					...
					if (component != null)
					{
						BuildLocationRule buildLocationRule = component.Def.BuildLocationRule;
						if ((uint)(buildLocationRule - 8) <= 2u)
						{
							flag2 = true;
						}
					}
				}
				if (!flag2 && (!permitUproots || !Uprootable.CanUproot(gameObject2) || gameObject2.HasTag(GameTags.BlockBuildOverPlantFeature)))
				{
			...
			{
				if (AttachmentSlotTag != GameTags.Rocket && Grid.Objects[num, 39] != null)
				{
			...
			{
				if (BuildLocationRule != BuildLocationRule.BuildingAttachPoint)
				{
					continue;
				}
				...
				{
					int cell2 = Grid.OffsetCell(cell, attachablePosition);
					BuildingAttachPoint component3 = gameObject5.GetComponent<BuildingAttachPoint>();
					if (!(component3 != null) || !component3.AcceptsAttachment(AttachmentSlotTag, cell2))
					{
						fail_reason = UI.TOOLTIPS.HELP_BUILDLOCATION_OCCUPIED;
						flag = false;
						break;
		...
		case BuildLocationRule.NotInTiles:
		{
			...
			{
				GameObject gameObject8 = Grid.Objects[cell, (int)ObjectLayer];
				if (gameObject8 != null)
				{
					...
					else
					{
						...
						if (component5 != null && component5.Def.ReplacementLayer != ReplacementLayer)
						{
							flag = false;
						}
					}
	...
	public bool IsValidPlaceLocation(GameObject source_go, int cell, Orientation orientation, bool replace_tile, out string fail_reason, bool restrictToActiveWorld)
	{
		...
		else if (BuildLocationRule == BuildLocationRule.OnBackWall)
		{
	...
	public static bool CheckFoundation(int cell, Orientation orientation, BuildLocationRule location_rule, int width, int height, Tag optionalFoundationRequiredTag = default(Tag))
	{
		...
		case BuildLocationRule.OnBackWall:
			return CheckBackWallFoundation(cell, width, height, orientation);
		case BuildLocationRule.OnWall:
	...
	public void PostProcess()
	{
		...
		if (!Deprecated)
		{
			Db.Get().TechItems.AddTechItem(PrefabID, Name, Effect, GetUISprite, RequiredDlcIds, ForbiddenDlcIds, POIUnlockable)?.AddSearchTerms(searchTerms);
		}
	}
...
===== KAnimGroupFile =====
DECOMPILE ERROR: Type KAnimGroupFile was not found in the module being decompiled, but only in Assembly-CSharp-firstpass
  Field MASTER_GROUP_FILE  0400007B KAnimGroupFile.MASTER_GROUP_FILE
  Field MAX_ANIMS_PER_GROUP  0400007C KAnimGroupFile.MAX_ANIMS_PER_GROUP
  Field groupfile  0400007D KAnimGroupFile.groupfile
  Field fileData  0400007E KAnimGroupFile.fileData
  Field groups  0400007F KAnimGroupFile.groups
  Field currentGroup  04000080 KAnimGroupFile.currentGroup
  Method DestroyInstance  06000127 KAnimGroupFile.DestroyInstance
  Method GetFilePath  06000128 KAnimGroupFile.GetFilePath
  Method GetGroupFile  06000129 KAnimGroupFile.GetGroupFile
  Method GetGroup  0600012A KAnimGroupFile.GetGroup
  Method GetGroupForHomeDirectory  0600012B KAnimGroupFile.GetGroupForHomeDirectory
  Method GetData  0600012C KAnimGroupFile.GetData
  Method Reset  0600012D KAnimGroupFile.Reset
  Method AddGroup  0600012E KAnimGroupFile.AddGroup
  Method AddAnimFile  0600012F KAnimGroupFile.AddAnimFile
  Method AddFile  06000130 KAnimGroupFile.AddFile
  Method AddAnimMod  06000131 KAnimGroupFile.AddAnimMod
  Method LoadGroupResourceFile  06000132 KAnimGroupFile.LoadGroupResourceFile
  Method LoadAll  06000133 KAnimGroupFile.LoadAll
  Method MapNamesToAnimFiles  06000134 KAnimGroupFile.MapNamesToAnimFiles
  Method DoMapNamesToAnimFiles  06000135 KAnimGroupFile.DoMapNamesToAnimFiles
  Method Load  06000136 KAnimGroupFile.Load
  Constructor .ctor  06000137 KAnimGroupFile..ctor
