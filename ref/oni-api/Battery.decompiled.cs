...
public class Battery : KMonoBehaviour, IEnergyConsumer, ICircuitConnected, IGameObjectEffectDescriptor, IEnergyProducer
{
	...
	private void OnTagsChanged(object data)
	{
		...
		{
			Game.Instance.circuitManager.Connect(this);
			GetComponent<KSelectable>().SetStatusItem(Db.Get().StatusItemCategories.Power, Db.Get().BuildingStatusItems.BatteryJoulesAvailable, this);
		}
		else
		{
			Game.Instance.circuitManager.Disconnect(this, isDestroy: false);
			GetComponent<KSelectable>().RemoveStatusItem(Db.Get().BuildingStatusItems.BatteryJoulesAvailable);
		}
	}
