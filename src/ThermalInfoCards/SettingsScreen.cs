...
namespace ThermalInfoCards
{
	...
	{
		private const string SettingsButtonName = "TT_SettingsButton";

		internal static MultiToggle ToggleTemplate;
		private static readonly System.Action[] Refreshers = new System.Action[6];
		...
		public static void Show()
		{
			...
			dialog.SetHeader(STRINGS.THERMAL_INFO_CARDS.SETTINGS_TITLE)
				.AddPlainText(STRINGS.THERMAL_INFO_CARDS.SETTINGS_HINT)
				.AddDefaultOK();
		...
		public static void AddModButton(object modsScreen)
		{
			...
			foreach (object row in rows)
			{
				...
				if (label != null)
					label.text = STRINGS.THERMAL_INFO_CARDS.SETTINGS_BUTTON;
		...
		private static void PopulateRows(InfoDialogScreen dialog)
		{
			...
			AddRow(body, toggleTemplate, labelTemplate, 0,
				STRINGS.THERMAL_INFO_CARDS.DISPLAY_ALL,
				STRINGS.THERMAL_INFO_CARDS.DISPLAY_ALL_TOOLTIP,
				() => ModSettings.AllUnitsStored,
				value => ModSettings.AllUnits = value);
			AddRow(body, toggleTemplate, labelTemplate, 1,
				STRINGS.THERMAL_INFO_CARDS.ONLY_THERMAL,
				STRINGS.THERMAL_INFO_CARDS.ONLY_THERMAL_TOOLTIP,
				() => ModSettings.OnlyOnThermalOverlay,
				value => ModSettings.OnlyOnThermalOverlay = value);
			AddRow(body, toggleTemplate, labelTemplate, 2,
				STRINGS.THERMAL_INFO_CARDS.HIDE_ELEMENT_CATEGORIES,
				STRINGS.THERMAL_INFO_CARDS.HIDE_ELEMENT_CATEGORIES_TOOLTIP,
				() => ModSettings.HideElementCategories,
				value => ModSettings.HideElementCategories = value);
			AddRow(body, toggleTemplate, labelTemplate, 3,
				STRINGS.THERMAL_INFO_CARDS.RESTRICT_SELECTION,
				STRINGS.THERMAL_INFO_CARDS.RESTRICT_SELECTION_TOOLTIP,
				() => ModSettings.UseBaseSelection,
				value => ModSettings.UseBaseSelection = value);
			AddRow(body, toggleTemplate, labelTemplate, 4,
				STRINGS.THERMAL_INFO_CARDS.FIRST_SELECTION_HOVER,
				STRINGS.THERMAL_INFO_CARDS.FIRST_SELECTION_HOVER_TOOLTIP,
				() => ModSettings.ForceFirstSelectionToHover,
				value => ModSettings.ForceFirstSelectionToHover = value);
			AddRow(body, toggleTemplate, labelTemplate, 5,
				STRINGS.THERMAL_INFO_CARDS.OVERRIDE_CARD_SIZE,
				STRINGS.THERMAL_INFO_CARDS.OVERRIDE_CARD_SIZE_TOOLTIP,
				() => ModSettings.OverrideCardSize,
				value => ModSettings.OverrideCardSize = value);
		}
		...
		private static void AddRow(
			...
			Action<bool> set)
		{
			GameObject row = new GameObject("TT_SettingsRow_" + index, typeof(RectTransform), typeof(HorizontalLayoutGroup), typeof(LayoutElement), typeof(Image));
