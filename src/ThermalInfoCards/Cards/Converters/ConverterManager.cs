...
public static class ConverterManager
{
    ...
    static ConverterManager()
    {
        ...
        AddConverter(
            HoverTooltip.ExportThermalMass,
            data => data is float value ? value : 0f,
            (_, values) => string.Format(
                TTS.THERMAL_MASS,
                ThermalUtil.DoScientific(values.Sum()),
                UI.UNITSUFFIXES.HEAT.KDTU.text.Trim(),
                GameUtil.GetTemperatureUnitSuffix()?.Trim()) + sumSuffix);

        AddConverter(
            HoverTooltip.ExportHeatEnergy,
            data => data is float value ? value : 0f,
            (_, values) => string.Format(
                TTS.HEAT_ENERGY,
                ThermalUtil.DoScientific(values.Sum()),
                UI.UNITSUFFIXES.HEAT.KDTU.text.Trim()) + sumSuffix);
    }
