...
public class InfoCard
{
    ...
    public void Draw(List<InfoCard> cards, int visCardIndex)
    {
        if (visCardIndex > 0)
        {
            ...
            SelectToolHoverTextCard hoverCard = SelectTool.Instance != null
                ? SelectTool.Instance.GetComponent<SelectToolHoverTextCard>()
                : null;
            TextStyleSetting titleStyle = hoverCard != null ? hoverCard.Styles_Title.Standard : null;
            var drawCount = new DrawActions.Text().Set(ti, titleStyle, Color.white, false);
