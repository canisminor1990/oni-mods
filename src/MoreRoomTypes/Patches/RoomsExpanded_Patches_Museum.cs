...
namespace MoreRoomTypes
{
	class RoomsExpanded_Patches_Museum
	{
		public static void AddRoom(ref RoomTypes __instance)
		{
			...
			RoomConstraintTags.AddStompInConflict(RoomConstraintTags.FindRoomType(__instance, "BanquetHall"), RoomTypes_AllModded.Museum);
			...
			if (Settings.Instance.PrivateBathroom.IncludeRoom)
				RoomConstraintTags.AddStompInConflict(RoomTypes_AllModded.PrivateBathroom, RoomTypes_AllModded.Museum);
		}
		...
		public static class ItemPedestalConfig_ConfigureBuildingTemplate_Patch
		{
			...
			{
				if (!Settings.Instance.Museum.IncludeRoom && !Settings.Instance.MuseumSpace.IncludeRoom)
					return;
