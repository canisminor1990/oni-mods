...
namespace MoreRoomTypes
{
	...
	{
		internal const string SettingsButtonName = "MRT_SettingsButton";
		internal static MultiToggle ToggleTemplate;
		static readonly List<System.Action> Refreshers = new List<System.Action>();
		...
		public static void Show()
		{
			...
			{
				Debug.LogWarning($"{Mod.Namespace}: cannot open settings: game dialog prefab is missing");
				return;
			}
			...
			dialog.SetHeader(STRINGS.SETTINGS.TITLE)
				.AddPlainText(STRINGS.SETTINGS.HINT)
				.AddOption(STRINGS.SETTINGS.ALL_ON, _ => SetAll(true))
				.AddOption(STRINGS.SETTINGS.ALL_OFF, _ => SetAll(false))
				.AddDefaultOK();
		...
		static void DisableHintRaycast(InfoDialogScreen dialog)
		{
		...
		static void PopulateRows(InfoDialogScreen dialog)
		{
			Refreshers.Clear();
			...
			{
				Debug.LogWarning($"{Mod.Namespace}: settings checkbox template was not found");
				return;
			}

			for (int i = 0; i < Settings.ToggleIds.Length; i++)
				AddRow(body, toggleTemplate, labelTemplate, Settings.ToggleIds[i]);
		}

		static void AddRow(Transform body, MultiToggle toggleTemplate, InfoScreenPlainText labelTemplate, string id)
		{
			GameObject row = new GameObject("MRT_SettingsRow_" + id, typeof(RectTransform), typeof(HorizontalLayoutGroup), typeof(LayoutElement), typeof(Image));
			...
			{
				bool on = Settings.Instance.IsRoomEnabled(roomId);
				if (toggle != null)
					toggle.ChangeState(on ? 1 : 0);
				if (loc != null)
				{
					loc.key = "";
					loc.text = Settings.Instance.LabelFor(roomId);
				}
			};
			refresh();
			...
			{
				Settings.Instance.SetRoomEnabled(roomId, !Settings.Instance.IsRoomEnabled(roomId));
				refresh();
			};
		...
		static Transform FindBody(InfoDialogScreen dialog)
		{
		...
		{
			Settings.Instance.SetAllRoomsEnabled(enabled);
	...
	internal static class ModsScreenPatch
	{
		...
		public static class ModsScreen_BuildDisplay_Patch
		{
			public static void Postfix(object __instance)
			{
				...
				{
					Debug.LogWarning($"{Mod.Namespace}: ModsScreen button failed: {ex.Message}");
				}
			}

			static void AddSettingsButton(object modsScreen)
			{
				...
				foreach (object row in rows)
				{
					...
					Transform existing = parent.Find(SettingsScreen.SettingsButtonName);
					if (existing != null)
					{
